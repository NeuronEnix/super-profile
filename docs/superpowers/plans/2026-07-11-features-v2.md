...
**Spec:** `docs/superpowers/specs/2026-07-11-features-v2-design.md` — read it first.

## Execution protocol (orchestrator + subagents)

- The orchestrator (Fable, this build) dispatches ONE subagent per task (Sonnet, thinking high). The subagent reads `CLAUDE.md`, the spec, and its own task section, then executes every step **except the git commit/push steps** — the orchestrator reviews the diff and commits.
...
Run: `cd backend && CI=true npx wrangler d1 migrations apply hyugorix --local`
...
- [x] **Step 4: Build + deploy.** From repo root: `pnpm --dir frontend build`, then `cd backend && CI=true npx wrangler d1 migrations apply hyugorix --remote && npx wrangler deploy`. Expected: migration 0005 applied remotely; deploy prints a version id.
...
Apply locally: `cd backend && CI=true npx wrangler d1 migrations apply hyugorix --local`
...
- [x] **Step 9: Green + deploy.** `cd backend && pnpm test` all pass; `pnpm --dir frontend build` (repo root); `cd backend && CI=true npx wrangler d1 migrations apply hyugorix --remote && npx wrangler deploy`.
...
- [x] **Step 8: Deploy + prod smoke.** Apply migration 0007 `--remote`, deploy, then browse the prod demo page with a real workspace key (orchestrator, in browser), click around, open the inbox, see the trail. **Commit + push**: `git add -A && git commit -m "feat(timeline): contact hyugorix panel — activity trail, last seen, history" && git push origin main`
...
- [x] **Step 6: Final commit + push.** `git add -A && git commit -m "docs: morning report, README features, decision log for v2" && git push origin main`
